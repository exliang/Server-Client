#
# Simple socket server example implementing the SI32P protocol
# A new command on SI32P protocol & in the reference server: LOWERCASE <string> 
# → should send back to the client the string it received but lowercase 
...
def check_hello(si32p_conn: SI32PConnection, cmd: str) -> bool:
    '''
...
def is_command_recognized(msg) -> bool:
    '''
    ...
    '''
    res = False
    # All the commands from the SI32P procotol supported by this server
    all_cmd = [si32p.SI32P_CLI_HELLO, si32p.SI32P_CLI_INVERT,
               si32p.SI32P_CLI_BYE] #SI32P_CLI_LOWERCASE
...
def process_command(msg, cmd) -> str:
    '''
...
def cmd_invert(msg: str) -> str:
    '''
    Implements the string invert using simple slicing
    '''
    return msg[::-1]#.lower()
...
if __name__ == "__main__":
    ...
    start_server(IPAddr, int(sys.argv[1]))  # need to run it w a port
